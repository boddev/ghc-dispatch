...
const task_tree_1 = require("./providers/task-tree");
const agent_tree_1 = require("./providers/agent-tree");
const skill_tree_1 = require("./providers/skill-tree");
const automation_tree_1 = require("./providers/automation-tree");
const approval_tree_1 = require("./providers/approval-tree");
const task_detail_1 = require("./panels/task-detail");
let sseDisconnect = null;
function activate(context) {
    const config = vscode.workspace.getConfiguration('dispatch');
    const apiUrl = config.get('apiUrl', 'http://localhost:7878');
    const refreshInterval = config.get('autoRefreshInterval', 5000);
    const client = new client_1.DispatchClient(apiUrl);
    // --- TreeView Providers ---
    const taskTree = new task_tree_1.TaskTreeProvider(client);
    const agentTree = new agent_tree_1.AgentTreeProvider(client);
    const skillTree = new skill_tree_1.SkillTreeProvider(client);
    const autoTree = new automation_tree_1.AutomationTreeProvider(client);
    const approvalTree = new approval_tree_1.ApprovalTreeProvider(client);
    ...
    // --- Commands ---
    context.subscriptions.push(vscode.commands.registerCommand('dispatch.refreshTasks', () => refreshAll()), vscode.commands.registerCommand('dispatch.createTask', async () => {
        const title = await vscode.window.showInputBox({ prompt: 'Task title', placeHolder: 'Fix the auth bug' });
        if (!title)
            return;
        const agent = await vscode.window.showQuickPick(['@general-purpose', '@coder', '@designer'], { placeHolder: 'Select agent' }) ?? '@general-purpose';
        const priority = await vscode.window.showQuickPick(['normal', 'low', 'high', 'critical'], { placeHolder: 'Select priority' }) ?? 'normal';
        try {
            const task = await client.post('/api/tasks', { title, agent, priority, createdBy: 'vscode' });
        ...
        try {
            const stats = await client.get('/api/stats');
            const memStats = await client.get('/api/memory/stats');
            const lines = [
        ...
        if (!query)
            return;
        try {
            const suggestions = await client.post('/api/memory/suggest', { message: query, channel: 'vscode' });
    ...
    }), vscode.commands.registerCommand('dispatch.openTaskDetail', (taskId) => {
        if (!taskId)
            return;
        (0, task_detail_1.showTaskDetail)(client, taskId);
    }), vscode.commands.registerCommand('dispatch.cancelTask', async (item) => {
        if (!item?.taskId)
            return;
        await client.post(`/api/tasks/${item.taskId}/cancel`);
        vscode.window.showInformationMessage(`Task ${item.taskId} cancelled`);
        taskTree.fetchTasks();
    }), vscode.commands.registerCommand('dispatch.retryTask', async (item) => {
        if (!item?.taskId)
            return;
        await client.post(`/api/tasks/${item.taskId}/retry`);
        vscode.window.showInformationMessage(`Task ${item.taskId} retried`);
        taskTree.fetchTasks();
    }), vscode.commands.registerCommand('dispatch.enqueueTask', async (item) => {
        if (!item?.taskId)
            return;
        await client.post(`/api/tasks/${item.taskId}/enqueue`);
        vscode.window.showInformationMessage(`Task ${item.taskId} enqueued`);
        taskTree.fetchTasks();
    }), vscode.commands.registerCommand('dispatch.approveApproval', async (item) => {
        if (!item?.approvalId)
            return;
        ...
        approvalTree.refresh();
    }), vscode.commands.registerCommand('dispatch.rejectApproval', async (item) => {
        if (!item?.approvalId)
            return;
        ...
        approvalTree.refresh();
    }), vscode.commands.registerCommand('dispatch.enableSkill', async (item) => {
        if (!item?.skillId)
            return;
        await client.post(`/api/skills/${item.skillId}/enable`);
        skillTree.refresh();
    }), vscode.commands.registerCommand('dispatch.disableSkill', async (item) => {
        if (!item?.skillId)
            return;
        ...
        if (choice.value === 'facts') {
            ...
            if (!q)
                return;
            const facts = await client.get(`/api/memory/facts?q=${encodeURIComponent(q)}`);
            ...
            const entities = await client.get('/api/memory/entities');
            ...
            if (picked) {
                const profile = await client.get(`/api/memory/profile/${picked.label}`);
                vscode.window.showInformationMessage(profile.profile || 'No profile data');
            }
        }
        else if (choice.value === 'episodes') {
            const episodes = await client.get('/api/memory/episodes?limit=10');
            ...
            const msgs = await client.get('/api/conversations?limit=30');
...
function showApprovalNotification(client, event, approvalTree) {
    const approvalId = event.approvalId;
    vscode.window.showWarningMessage(`⚠️ Approval required for task ${event.taskId}`, 'Approve', 'Reject').then(async (action) => {
        if (!action)
            return;
