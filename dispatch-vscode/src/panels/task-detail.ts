...
export function showTaskDetail(client: DispatchClient, taskId: string) {
  ...
  const apiBase = (client as any).baseUrl ?? 'http://localhost:7878';

  panel.webview.onDidReceiveMessage(async (msg) => {
    try {
      if (msg.command === 'cancel') await client.post(`/api/tasks/${msg.taskId}/cancel`);
      if (msg.command === 'retry') await client.post(`/api/tasks/${msg.taskId}/retry`);
      if (msg.command === 'enqueue') await client.post(`/api/tasks/${msg.taskId}/enqueue`);
